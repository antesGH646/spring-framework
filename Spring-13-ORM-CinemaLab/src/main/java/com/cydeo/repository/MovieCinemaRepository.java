...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema, Long> {
    ...
    List<MovieCinema> findAllByMovie_NameContainingIgnoreCase(String name);
    ...
    List<MovieCinema> findAllMovieCinemasByDateTimeAfter(@Param("date") LocalDateTime dateTime);
    // ------------------- Native QUERIES ------------------- //

    //Write a native query to count all movie cinemas by cinema id
    @Query(value = "SELECT COUNT(*) FROM movie_cinema WHERE cinema_id = ?1",nativeQuery = true)
    Integer countAllMovieCinemaById();
    //Write a native query that returns all movie cinemas by location name
    @Query(value = "SELECT * FROM movie_cinema mc JOIN cinema c ON mc.cinema_id = c.id " +
            "JOIN location l ON c.location_id = l.id WHERE l.name = ?1"
            ,nativeQuery = true)
    List<MovieCinema> findAllMovieCinemasByLocationName(@Param("name") String name);
}
