...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    List<Movie> findAllMovieByStateAndType(MovieState state, MovieType type);
    // ------------------- JPQL QUERIES ------------------- //

    //Write a JPQL query to list all movies between a range of prices
   // @Query("select m from Movie m where m.price >= ?1 and m.price <= ?2")
    @Query("select m from Movie m where m.price between ?1 and ?2")
    List<Movie> retrieveAllMovieByPriceBetween(@Param ("price1") BigDecimal price1, @Param("price2") BigDecimal Price2);
    //Write a JPQL query that returns all movie names
    @Query("select  m.name from Movie m")
    List<String> fetchAllMovieByName();
    // ------------------- Native QUERIES ------------------- //
    ...
    List<Movie> fetchAllMovieByPriceBetween(@Param ("price1") BigDecimal price1, @Param("price2") BigDecimal Price2);
    //Write a native query to return all movies where duration exists in the range of duration
    @Query(value = "SELECT * FROM movie WHERE duration IN ?1",nativeQuery = true)
    List<Movie> retrieveAllMoviesByDurationInRange(@Param("durations") List<Integer> durations);
